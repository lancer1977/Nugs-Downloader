...
public sealed class LiveNugsWorkflowTests
{
    ...
    private const string DefaultLiveUrl = "https://play.nugs.net/release/35892";

    [Fact]
    [Trait("Category", "LiveNugs")]
    public async Task DownloadWorkflow_CompletesARepresentativeLiveNugsDownload()
    {
        ...
        Assert.False(string.IsNullOrWhiteSpace(secrets.Username), "NUGS_USERNAME is required for live workflow tests.");
        Assert.False(string.IsNullOrWhiteSpace(secrets.Password), "NUGS_PASSWORD is required for live workflow tests.");
        ...
        var request = new StartDownloadRequest(
            null,
            provider.Id,
            new Uri(GetValue(EnvironmentVariables(), "NUGS_LIVE_DOWNLOAD_URL") ?? DefaultLiveUrl),
