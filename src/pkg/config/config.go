...
func readConfig() (*Config, error) {
	...
	// 1. Try ~/.config/nugs-downloader/config.json
	homeDir, errHome := os.UserHomeDir()
	if errHome == nil {
		homeConfigPath := filepath.Join(homeDir, ".config", "nugs-downloader", configFilename)
	...
	// If not found anywhere, create a default one in ~/.config/nugs-downloader
	if err != nil && os.IsNotExist(err) && errHome == nil {
		homeConfigDir := filepath.Join(homeDir, ".config", "nugs-downloader")
